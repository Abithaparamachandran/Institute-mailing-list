	    ...
	    if (isset($_POST['search_date'])) {
	        ...
	        elseif (isset($_POST['month_year'])) {
			        $monthYear = $_POST['month_year']; 
				        $entrySql = "SELECT * FROM seminarorg WHERE DATE_FORMAT(modifiedtimestamp, '%Y-%m') = '$monthYear' ORDER BY modifiedtimestamp DESC";
				    }
} else {
    ...
    <div class="container">
        ...
        <form method="post" class="text-center">
            <div class="form-group">
                <input type="date" name="search_date" class="form-control" value="<?php echo $searchDate; ?>" style="display: inline-block; width: auto; margin-bottom: 20px;">
        ...
        <form method="post" class="text-center">
            <div class="form-group">
                <select name="month_year" class="form-control" style="display: inline-block; width: auto; margin-bottom: 20px;">
                    <?php
                   
                    for ($m = 1; $m <= 12; $m++) {
			                            $month = str_pad($m, 2, '0', STR_PAD_LEFT);
						                            $year = date('Y');
						                            echo "<option value='$year-$month' " . (($monthYear == "$year-$month") ? "selected" : "") . ">$year-$month</option>";
									                        }
                    ?>
                </select>
